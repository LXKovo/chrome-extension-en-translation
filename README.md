...
- **右侧侧边栏**：点击图标在浏览器右侧打开（`chrome.sidePanel`），高度铺满整个页面、可跨标签页保持打开与状态，长文翻译过程中不会因切换或误点中断。
...
2. 点击工具栏的插件图标，在右侧侧边栏中点击「一键翻译」。
3. 插件依次执行：提取正文 → 交给后台流式翻译 → 逐字展示结果。
4. 翻译完成后可点击「下载」导出 `.md` 文件；关闭侧边栏后再打开仍可查看上次结果。
...
├── sidepanel/    # 侧边栏 UI（React）：主视图 / 设置视图、打字机展示、md-wx 渲染
│   ├── components/
│   └── hooks/
└── shared/       # 跨层共享：类型、常量、消息协议
```

三个运行上下文（background / content / sidepanel）相互独立，仅通过 `src/shared/` 与 `chrome.runtime` 消息通信。
...
- 最小权限：仅申请 `storage`、`activeTab`、`scripting`、`sidePanel` 及 API 主机访问权限。
